 ...
 */
/*--------------------------------------------------------------------*/
public class BranchBoundFeatureMiner extends MinerAbs implements Runnable {
	...
	public boolean completeSearch = false; // if completeSearch is set, all
											// subgraph would be enumerated with
											// pruning rule, the supp will not
											// affects
	...
	// if existed Subgraphs is provided, we only return the subgraph which is
	// not included in existedSubgraphs
	ArrayList<Graph> existedSubgraphs;
	public double alpha = 1;
	public double beta = .1;
	public double gamma = 1;
	String mineMethod = "gSemi"; // gMLC
	private Matrix LaplacianMatrix;
	private Matrix LaplacianMatrix_Pos;
	...
	public BranchBoundFeatureMiner() { /* --- create a substructure finder */
	...
	protected boolean output(Fragment frag) throws IOException { /*
		...
		/*
		 * DecisionStump stump = this.gain(sub); if(this.maxGainGraph == null){
		 * maxGainGraph = stump; } else{ if(stump.getGain() >
		 * maxGainGraph.getGain()) maxGainGraph = stump; else{ // use the
		 * upperbound to prune candidates double upperbound =
		 * this.upperBoundGain(sub); if(upperbound < maxGainGraph.getGain())
		 * return false; } }
		 */
	...
	private boolean recEmbed(Fragment frag, int depth) throws IOException { /*
		...
		if (stump.getGain() >= this.wSupp + double_precision && isUnique(stump)) { 
			//candiate have discriminative score large enough and has not been selected before
			if (maxStumps.size() < this.numStump) {
				...
				//update current minimum score
				this.wSupp = maxStumps.iterator().next().getGain(); 
			}
			// System.out.println("Min weight score:"+this.wSupp);

		} else {
			// if the boundScore of a graph is greater than the mimimum score,
			// than it is a potential node.
			// boundScore = this.upperBoundGain(frag);
			if (boundScore - double_precision < wSupp) { // only consider those
															// may have large
															// enough subgraphs
	...
	private int searchEmbed() throws IOException { /*
		...
		if (this.seed != null) { /* if there is a seed structure */
			...
			for (i = 0; i < this.coder.size(); i++) {
				...
				/*
				 * this.log.print("\nprocessing " + s.substring(0, 8)); if
				 * ((this.mode & VERBOSE) != 0) System.out.println(); if verbose
				 * output, start new line
	...
	public void init(String args[]) throws IOException { /*
		...
		((idsfn == null) || idsfn.equals("")) ? null : new FileWriter(idsfn));
	...
	protected void mine() throws IOException { /* --- run substructure search */
		...
		t = System.currentTimeMillis() - t;
		this.log.println("[" + k + " visited nodes(s), obtained "
				+ this.maxStumps.size() + " optimal subgraph(s)] done ["
				+ (t / 1000.0) + "s].");
	} /* mine() */
	...
	public ArrayList<Graph> readGraphs(String file, int mod) throws IOException {
		...
		double c1 = 0, c2 = 0;
		GraphReader reader = GraphReader.createReader(new FileReader(file),
				mod, format);
	...
	public double[] getIndicateVector(Fragment frag) {
		...
		if (sup != frag.getSupport()) {
			System.err.println("Error: " + this.getClass() + "->Cal support:"
					+ sup + "\tReal Support:" + frag.getSupport());
		}

		return vector;
	}

	public DecisionStump getMaxGainDecisionStump(ArrayList<Graph> graphs,
			int support) {
		/*
		 * String opt = "-t1.0 -s"+support+" -K"; try {
		 * 
		 * for(int i = 0; i < graphs.size(); i++){ NamedGraph ng = new
		 * NamedGraph((NamedGraph)graphs.get(i)); this.addGraph(ng); }
		 * 
		 * 
		 * //System.out.println("Number of graphs in focus:"+graphs.size());
		 * initMiner(opt.split(" ")); run(); find substructures, and //stats();
		 * 
		 * } catch (IOException e) { e.printStackTrace(); }
		 */
		TreeSet<DecisionStump> stump = getMaxGainDecisionStumps(graphs, null,
				support, 0.01, 1, 1);
	...
	public TreeSet<DecisionStump> getMaxGainDecisionStumps_LU(
			ArrayList<Graph> trainGraphs, ArrayList<Graph> uGraphs,
			ArrayList<Graph> excludedSubs, int support, double ws,
			int numStump, int maxPad, boolean b) {
		return getMaxGainDecisionStumps(trainGraphs, excludedSubs, support, wSupp,
				numStump, maxPad);
	}
	
	@Override
	public TreeSet<DecisionStump> getMaxGainDecisionStumps(
			ArrayList<Graph> graphs, ArrayList<Graph> excludedSubs,
			int support, double wSupp, int numStumps, int maxSize,
			boolean remine) {
		// TODO Auto-generated method stub
		return getMaxGainDecisionStumps(graphs, excludedSubs, support, wSupp,
				numStumps, maxSize);
	}
	 ...
	 *            Training graphs
	 * @param support
	 *            percentage (1-100) of support for subgraphs, if set to -1, no
	 *            support is setting, algorithm will enumerate all subgraph with
	 *            pruning rule
	 * @param wSupp
	 *            minimal weighed score for subgraphs \sum_i^n
	 *            y_i*lamda_i*h(x_i)
	 * @param numStumps
	 *            maximal number of subgraphs to return
	 * @param maxSize
	 *            maximal size of subgraphs (number of nodes)
	 * @return
	 */
	public TreeSet<DecisionStump> getMaxGainDecisionStumps(
			ArrayList<Graph> graphs, ArrayList<Graph> excludedSubs,
			double support, double wSupp, int numStumps, int maxSize) {
	 ...
	 */
	public TreeSet<DecisionStump> getMaxGainDecisionStumps(
			ArrayList<Graph> graphs, ArrayList<Fragment> frequent,
			double wSupp, int num) {
	...
	public void initMiner(String args[]) throws IOException { /*
		...
		/*
		 * rdr = GraphReader.createReader(new FileReader(datfn),
		 * GraphReader.GRAPHS, input); if (rdr == null) create the graph reader
		 * throw new IOException("error: invalid format " + input);
		 * this.setInput(rdr); set the graph reader if (rdr instanceof
		 * NEListReader) { output = "list"; excl = null; } wrt =
		 * GraphWriter.createWriter(new FileWriter(subfn), GraphWriter.SUBS,
		 * output); if (wrt == null) create the graph writer throw new
		 * IOException("error: invalid format " + output); this.setOutput(wrt,
		 * set the graph writer ((idsfn == null) || idsfn.equals("")) ? null :
		 * new FileWriter( idsfn));
		 */
	...
	public double upperBoundGain(Fragment frag) {
		...
		while (graph != null) {
			...
			boolean contain = GraphUtils.subgraphIsomorphism(
					(Graph) subgraph.clone(), (Graph) graph.clone());
			if (contain) {
				support++;
				if (classValue == 1) {
					upV1 += graph.weight;
		...
		if (support != frag.getSupport()) {
			System.err.println("Upper->Cal support:" + support
					+ "\tReal Support:" + frag.getSupport());
		}

		// System.out.println("Upper bound:"+(value1 - sumOfWeight)+"\t"+(value1
		// - sumOfWeight));
		return Math.max(2 * upV1 - sumOfWeight, 2 * upV2 + sumOfWeight);
	}

	private DecisionStump gainStump(Fragment frag) {
		
		Graph sub = (Graph) frag.getGraph();
		NamedGraph graph = graphs;
		
		double f[] = new double[this.cnts[0]];
		int i = 0, support = 0;
		while (graph != null) {
			Graph ng = graph;
		...
		if (support != frag.getSupport()) {
			try {
				System.err.println(this.getClass() + ":Cal support:" + support
						+ "\tReal Support:" + frag.getSupport());
				throw new Exception(this.getClass() + ":Cal support:" + support
						+ "\tReal Support:" + frag.getSupport());
		...
		double score = F.transpose().times(LaplacianMatrix).times(F).normF();
		double boundscore = F.transpose().times(LaplacianMatrix_Pos).times(F).normF();
		
		return new DecisionStump(sub, 1, score, boundscore,
				frag.getSupport(), zeroOneRepresentation);
	...
	public DecisionStump gainStump_LU(Fragment frag) {
		...
		while (graph != null) {
			Graph ng = graph;
			/*
			 * Graph ng = (Graph) graph.clone(); ng.maskTypes(GraphUtils.masks);
			 * ng.prepare();
			 */
		...
		// check support
		if (support != frag.getSupport()) {
			System.err.println("Cal support:" + support + "\tReal Support:"
					+ frag.getSupport());
		}
		...
		if (value1 > value2) {
			return new DecisionStump(sub, 1, value1, upScore,
					frag.getSupport(), zeroOneRepresentation);
		} else
			return new DecisionStump(sub, -1, value2, upScore,
					frag.getSupport(), zeroOneRepresentation);
	}
	...
	public void evaluateStumpList(ArrayList<Graph> graphs,
			ArrayList<Graph> ugraphs, DecisionStump stumpt) {
		...
		if (supp != stumpt.getSupport()) {
			System.err.println("Error for subgraph checking!support:" + supp
					+ "\tReal_Support:" + stumpt.getSupport());
			System.exit(0);
		}
		System.out.println("Gain:" + stumpt.getGain() + "\tACC:" + acc / sum
				+ "\t(" + acc + "/" + sum + ")\tsupport:" + supp
				+ "\tReal_Support:" + stumpt.getSupport());
	}
	...
	public void setSubgraphMiningMethod(String mineMethod,
			ArrayList<Graph> trainGraphs) {

		this.mineMethod = mineMethod;
		if(mineMethod.equalsIgnoreCase("gSemi")){
			System.out.println("Mining using gSemi method!");
			computeGSEMIMatrix(trainGraphs);
		}
		else{
			System.out.println("Mining using gMLC method!");

			computeGMLCMatrix(trainGraphs);
		}


	}

	public void computeGSEMIMatrix(ArrayList<Graph> graphs) {
		Matrix W = createWeightMatrix(graphs);
		Matrix D = createDMatrix(W);
		LaplacianMatrix = D.minus(W);
		LaplacianMatrix_Pos = createLMatrixPos(LaplacianMatrix);
	}
	
	public void computeGMLCMatrix(ArrayList<Graph> graphs) {
		
		Matrix H = createHMatrix(graphs.size());
		Matrix L = createLMatrix(graphs);
		

		LaplacianMatrix = H.times(L).times(H);
		LaplacianMatrix_Pos = createLMatrixPos(LaplacianMatrix);
	}
	
	private Matrix createLMatrix(ArrayList<Graph> graphs) {
		int n = graphs.size();
		Matrix m = new Matrix(n, n);
		for(int i = 0; i < n; i++){
			NamedGraph g1 = (NamedGraph) graphs.get(i);
			for (int j = 0; j < n; j++){
				NamedGraph g2 = (NamedGraph) graphs.get(j);
				m.set(i, j, g1.getValue() * g2.getValue());
			}
		}
	...
	private Matrix createWeightMatrix(ArrayList<Graph> graphs) {

		double C = 0; // cannot link coefficient
		double M = 0; // must link coefficient
		double Du = 0; // Unalbeled coefficient

		ArrayList<Double> duWei = new ArrayList<Double>();

		// Caculate the number of postive, negative, and unlabeled graphs
		double pos = 0, neg = 0, unlabel = 0;
		for (int i = 0; i < graphs.size(); i++) {
			NamedGraph iG = (NamedGraph) graphs.get(i);
			if (!iG.getLabel()) {
				unlabel += 1;// iG.weight;
				duWei.add(iG.weight);
			} else if (iG.getLabel() && iG.getValue() == 1) {
				pos += 1;// iG.weight;
			} else {
				neg += 1;// iG.weight;
			}
		...
		System.out.println("Number of Unlabel:" + unlabel);

		// get must link
		M = pos * (pos - 1) / 2 + neg * (neg - 1) / 2;
		...
		System.out.println("Number of Mustlink:" + M);
		System.out.println("Number of Cannotlink:" + C);
		System.out.println("Number of Du:" + Du);

		double[][] w = new double[graphs.size()][graphs.size()];
		for (int i = 0; i < graphs.size(); i++) {

			NamedGraph iG = (NamedGraph) graphs.get(i);
			for (int j = 0; j < graphs.size(); j++) {
				NamedGraph jG = (NamedGraph) graphs.get(j);
				...
				// if it is labeled
				boolean l1 = iG.getLabel();
				boolean l2 = jG.getLabel();

				int class1 = (int) iG.getValue();
				int class2 = (int) jG.getValue();

				if (l1 && l2 && class1 * class2 == -1) { // cannot link
					w[i][j] = alpha / C;
				} else if (l1 && l2 && class1 * class2 == 1) { // must link
					w[i][j] = -1 * beta / M;
				}

				else if (!l1 && !l2) {// both unlabeled
					w[i][j] = 1.0 * gamma / (Du);
				} else {
					w[i][j] = 0;
				}
	...
	private Matrix createLMatrixPos(Matrix L) {
		Matrix m = new Matrix(L.getRowDimension(), L.getColumnDimension());
	...
	private Matrix createHMatrix(int n) {
		Matrix H = Matrix.identity(n, n);
		for (int i = 0; i < H.getRowDimension(); i++) {
			for (int j = 0; j < H.getRowDimension(); j++) {
				H.set(i, j, H.get(i, j)-1/n);
			}
		}
		return H;
	}
	
	private Matrix createDMatrix(Matrix w) {
		Matrix d = Matrix.identity(w.getRowDimension(), w.getColumnDimension());
	...
	public static void main(String args[]) { /* --- main function */
		...
		BranchBoundFeatureMiner miner = new BranchBoundFeatureMiner(); /*
																		 * substructure
																		 * miner
																		 */
		...
		try { /* run substructure search */
			/*
			 * miner.init(args); initialize the miner, miner.run(); find
			 * substructures, and miner.stats();
			 */
			GraphUtils.setGraphType(DataConvert.Chemical);
			// ArrayList<Graph> graphs =
			// miner.readGraphs("data/175-balance800.sdf", GraphReader.GRAPHS);
			// ArrayList<Graph> graphs =
			// miner.readGraphs("ptc_data/PTC_pn_MM.smi", GraphReader.GRAPHS);
			ArrayList<Graph> graphs = miner.readGraphs("ptc_data/OKD_pn.sdf",
					GraphReader.GRAPHS);
			// set weight
			/*
			 * for(int i = 0; i<graphs.size(); i++){
			...
			miner.setSubgraphMiningMethod("gMLC", train);

			System.out.println("Number of labeled graphs:" + train.size());
			TreeSet<DecisionStump> stump = miner.getMaxGainDecisionStumps(
					train,  null, -1.0, 0, 20, 10);
			...
			miner = new BranchBoundFeatureMiner();
			miner.setAlreadySubGraphs(select);
			miner.setSubgraphMiningMethod("gSemi", train);

			TreeSet<DecisionStump> stump2 = miner.getMaxGainDecisionStumps(
					train, null, -1, 0, 20, 10);
