 ...
 */
/*--------------------------------------------------------------------*/
public class CopyOfGraphBoostMinerGraphsMtask extends MinerAbs implements Runnable {
	...
	public CopyOfGraphBoostMinerGraphsMtask() { /* --- create a substructure finder */
...
/*		DecisionStump stump = this.gain(sub);
	...
	//	 gain upperbound pruning, add by Shirui 6-11-2012 
		...
		if(stump.getGain() >= this.wSupp + double_precision && isUnique(stump)){ //candidates have score large enough, and is not selected before
			...
			if(boundScore -double_precision< wSupp){ //only consider those may have large enough subgraphs
...
/*		rdr = GraphReader.createReader(new FileReader(datfn),
	...
	public DecisionStump gainStumpMTaskFinal(Fragment frag){
...
/*			ng.maskTypes(GraphUtils.masks);
		...
		if("l1".equals(this.regularizationStr)){
			...
			finalGain = Math.abs(value1);
			finalUpScore  = Math.abs(upScore);
			//finalGain = MathUtils.sum(gainValues);;
		}else{// l2 
			//System.out.println("l2 regularzion");
			// norm of sub
			//finalGain = l2_norm(gainValues);
/*			finalUpScore = l2_norm(upScores);
			finalGain = l2_norm(gainValues);
			finalUpScore = l2_norm(upScores);*/
	...
	//	Graph subgraph = (Graph) frag.getGraph();
		...
		return new DecisionStump(sub, 1, finalGain,finalUpScore,frag.getSupport(),zeroOneRepresentation);
/*		if(value1 > value2){
...
/*	    sub.maskTypes(GraphUtils.masks); 
	...
	/*			ng.maskTypes(GraphUtils.masks);
		...
		if("l1".equals(this.regularizationStr)){
			...
			finalGain = MathUtils.sum(gainValues);
			finalGain = Math.abs(finalGain);
			finalUpScore  = MathUtils.sum(upScores);
			//finalGain = MathUtils.sum(gainValues);;
		}else{// l2 
			//System.out.println("l2 regularzion");
			// norm of sub
			...
			//norm of all
			//finalGain = Math.sqrt(finalGain);
			//finalUpScore = Math.sqrt(finalUpScore);
		}
...
/*			Graph ng = (Graph) graph.clone();
	...
	public static void main(String args[]) { /* --- main function */
		...
		CopyOfGraphBoostMinerGraphsMtask miner = new CopyOfGraphBoostMinerGraphsMtask(); /* substructure miner */
...
/*			miner.init(args);  initialize the miner, 
	...
	/*		for(int i = 0; i<graphs.size(); i++){
			...
			miner = new CopyOfGraphBoostMinerGraphsMtask();
