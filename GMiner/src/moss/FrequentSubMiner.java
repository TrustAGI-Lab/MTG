 ...
 */
/*--------------------------------------------------------------------*/
public class FrequentSubMiner implements Runnable {
	...
	/**Add by Shirui*/
	public ArrayList<FrequentGraph> frequentSubgraphs;
	...
	public FrequentSubMiner() { /* --- create a substructure finder */
		...
		frequentSubgraphs = new ArrayList<FrequentGraph>();
	} /* Miner() */
	...
	protected boolean output(Fragment frag) throws IOException { /*
		...
		sub = frag.getGraph(); 
		frequentSubgraphs.add(new FrequentGraph(sub, frag.getSupport()));
...
/*------------------------------------------------------------------*/
	...
	public ArrayList<FrequentGraph> getFrequentSubgraphs(ArrayList<Graph> graphs, double support){
		String opt = "-t1.0 -s"+support+" -K -x"; //-K is important, for subgraph isomorphism checking
		try {
			
			for(int i = 0; i < graphs.size(); i++){
				this.addGraph((NamedGraph) new NamedGraph(graphs.get(i)).clone());
			}
	...
	public ArrayList<FrequentGraph> getFrequentSubgraphs(
			ArrayList<Graph> graphs, int support, int minSize) {
		String opt = "-t1.0 -s"+support+" -K -x -m"+minSize; //remove -K
		try {
	...
	public ArrayList<FrequentGraph> getFrequentSubgraphs(
			ArrayList<Graph> graphs, int support, int minSize, int maxSize) {
		String opt = "-t1.0 -s"+support+" -K -x -m"+minSize+" -n"+maxSize;
		try {
	...
	public void initMiner(String args[]) throws IOException { /*
		...
		String subfn = "moss.sub";/* name of substructure file */
		String idsfn = null; /* name of identifier file */
		double split = 10; /* threshold for split */
		boolean invert = false; /* whether to invert the split */
		double psupp = 10.0; /* minimum support of embedding */
		double pcomp = 2.0; /* maximum support in complement */
		int smode = DEFAULT; /* search mode and support type */
...
/*		rdr = GraphReader.createReader(new FileReader(datfn),
	...
	public static void main(String args[]) { /* --- main function */
		FrequentSubMiner miner = new FrequentSubMiner(); /* substructure miner */
		try { /* run substructure search */
/*			miner.init(args);  initialize the miner, 
			miner.run();  find substructures, and 
			miner.stats();*/
			ArrayList<Graph>  graphs = miner.readGraphs("data/1-balance500.sdf", GraphReader.SEMIGRAPHS);
			System.out.println("Number of graphs:"+graphs.size());
			ArrayList<FrequentGraph>frequentFeatures = miner.getFrequentSubgraphs(graphs, 50);
			
			
			
			int count[] = new int[10];
			for(FrequentGraph fre : frequentFeatures){
				System.out.println(new SMILES().describe(fre.getGraph()));
				int c = fre.getGraph().getEdgeCount();
				count[c]++;
			}
			
			System.out.println(Utils.arrayToString(count));
			
			System.out.println(miner.frequentSubgraphs.size());
