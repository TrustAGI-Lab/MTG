 ...
 */
/*--------------------------------------------------------------------*/
public class GraphBoostMiner implements Runnable {
	...
	private boolean zeroOneRepresentation=false;
	...
	public GraphBoostMiner() { /* --- create a substructure finder */
	...
	//	 gain upperbound pruning, add by Shirui 6-11-2012 
		...
		if(this.maxGainGraph == null){
			...
			if(stump.getGain() > maxGainGraph.getGain())
				...
				double boundScore = this.upperBoundGain(frag);
				//System.out.println("max:"+maxGainGraph.getGain()+"\tupperbound:"+boundScore);
				if(boundScore < this.maxGainGraph.getGain()){
...
/*		rdr = GraphReader.createReader(new FileReader(datfn),
	...
	public double upperBoundGain(Fragment frag){
		...
		return Math.max(2*value1 - sumOfWeight, 2*value2 + sumOfWeight);
	}
	...
	public DecisionStump gainStump(Fragment frag){
		...
		NamedGraph graph = graphs;
		int support = 0;
		Graph subgraph = (Graph) frag.getGraph().clone();
		while(graph!=null){
			
			//mapping calss value [0,1] to [-1,1];
			double classValue = graph.getValue();
			
			boolean sub = GraphUtils.subgraphIsomorphism(
					subgraph, (Graph)graph.clone());
			if(sub){
				support++;
				value1 +=  graph.weight * classValue * 1;
				value2 +=  graph.weight * classValue * -1;
			}
			else{
				value1 += graph.weight * classValue * -1;
				value2 +=  graph.weight * classValue * 1;
		...
		if(value1 > value2){
			return new DecisionStump(subgraph, 1, value1,0,frag.getSupport(),zeroOneRepresentation);
		}
		else return new DecisionStump(subgraph, -1, value2,0, frag.getSupport(),zeroOneRepresentation);
	}
	...
	public void evaluateStumpList(ArrayList<Graph> graphs){
		...
		for(Graph ng: graphs){
			NamedGraph graph = (NamedGraph) ng;
			double value = graph.getValue();
			double predict = this.maxGainGraph.classifyGraph((Graph) graph.clone());
			if(value == predict)
				acc ++;
			
			if(GraphUtils.subgraphIsomorphism(maxGainGraph.getGraphFeature(), graph)){
				supp++;
			}
		...
		System.out.println("ACC:"+acc/sum +"\t("+acc+"/"+sum+")\tsupport:"+supp);
	}
	...
	public static void main(String args[]) { /* --- main function */
		GraphBoostMiner miner = new GraphBoostMiner(); /* substructure miner */
		try { /* run substructure search */
/*			miner.init(args);  initialize the miner, 
			miner.run();  find substructures, and 
			miner.stats();*/
			ArrayList<Graph>  graphs = miner.readGraphs("data/1-balance800.sdf", GraphReader.SEMIGRAPHS);
			System.out.println("Number of graphs:"+graphs.size());
			DecisionStump stump = miner.getMaxGainDecisionStump(graphs, 30);
			System.out.println(stump.getGain());
			
			//evaluate
			miner.evaluateStumpList(graphs);
			
			//miner.evaluateStump();
			//System.out.println(stump.getClassValue());
			//System.out.println("graph\n"+stump.getGraphFeature());
