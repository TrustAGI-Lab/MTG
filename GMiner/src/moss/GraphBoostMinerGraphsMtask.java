 ...
 * 
 * @author Christian Borgelt, modified By Shirui
 * @since 2002.03.11
 */
/*--------------------------------------------------------------------*/
public class GraphBoostMinerGraphsMtask extends MinerAbs implements Runnable {

	/*------------------------------------------------------------------*/
	/* constants: version information */
	/*------------------------------------------------------------------*/
	/** the program description */
	public static final String DESCRIPTION = "Boosting Graph Stumps (based on MoSS)";
	...
	//add by shirui
	public DecisionStump maxGainGraph;
	
	public TreeSet <DecisionStump> maxStumps;
	
	public int numStump = 10;
	
	public ArrayList<Fragment> frequent;
	
	public double wSupp= 0.1;//weighted support
	
	public boolean saveFrequent = false;
	
	public boolean completeSearch = false; // if completeSearch is set, all subgraph would be enumerated with pruning rule, the supp will not affects
	
	private boolean uiversumGraphs = false; //Including universum Graphs?
	
	double double_precision = 1E-5;
	
	float valueForUniversum = 0;
	
	// if existed Subgraphs is provided, we only return the subgraph which is not included in existedSubgraphs
	ArrayList<Graph> existedSubgraphs;
	private int[] taskSize;
	
	public String regularizationStr = "l1";
	public double mtaskGamma;
	public double mtaskVarepsilon;
	...
	public GraphBoostMinerGraphsMtask() { /* --- create a substructure finder */
		...
		maxStumps = new TreeSet<DecisionStump>();
		frequent = new ArrayList<Fragment>();
		
		existedSubgraphs = new ArrayList<Graph>();
		//zeroOneRepresentation = true;
	} /* Miner() */
	...
	protected boolean output(Fragment frag) throws IOException { /*
		...
		//If we  save the frequent subgraphs
		if(this.saveFrequent){
			frequent.add(frag);
		}
		
		/* --- write substructure file --- */
		
/*		DecisionStump stump = this.gain(sub);
		if(this.maxGainGraph == null){
			maxGainGraph = stump;
		}
		else{
			if(stump.getGain() > maxGainGraph.getGain())
				maxGainGraph = stump;
			else{
				// use the upperbound to prune candidates
				double upperbound = this.upperBoundGain(sub);
				if(upperbound < maxGainGraph.getGain())
					return false;
			}
		}*/
		

		return saveFrequent; /* return 'fragment was reported' */
	} /* output() */
	...
	//	 gain upperbound pruning, add by Shirui 6-11-2012 
		
		//double inv[] = this.getIndicateVector(frag);
		DecisionStump stump = null;
		if(this.uiversumGraphs)
			stump = this.gainStump_LU(frag); //current stump
		else{
			//stump = this.gainStump(frag); //current stump
			stump = this.gainStumpMTask(frag);
		}
		
		
		//System.out.println(frag.cover.length+":Supp:"+frag.getSupport());
		double boundScore =  stump.getUpperScore(); //upper bound score
		DecisionStump minStump = null; //current minimum stump
		
		double secondUpScore = stump.getSecondUpscores();
		
		//double minScore = Double.MIN_VALUE;
		
		if(stump.getGain() >= this.wSupp + double_precision && isUnique(stump) 
				&&stump.isValidStump() //norm score great than gammas
				){ //candidates have score large enough, and is not selected before
			if(	maxStumps.size() < this.numStump){
				maxStumps.add(stump);
			}
			else{//larger than the number of required stumps			
				minStump = maxStumps.iterator().next();
				if(stump.getGain() - minStump.getGain() >= double_precision){
					maxStumps.add(stump);
					
					//REMOVE THE MINIMUM ONE
					Iterator<DecisionStump> it = maxStumps.iterator();
					it.next();
					it.remove();
				}
				//this.wSupp = maxStumps.iterator().next().getGain(); //update current minimum score
			}
			
			
			if(maxStumps.size() >0){
				DecisionStump st = maxStumps.iterator().next();
				this.wSupp = st.getGain();
			}
			//System.out.println("Min weight score:"+this.wSupp);
			
		}else{
			//if the boundScore of a graph is greater than the mimimum score, than it is a potential node.
			//boundScore = this.upperBoundGain(frag);
			if(boundScore -double_precision< wSupp 
			&&secondUpScore < this.mtaskGamma		//norm score should great gammas
			){ //only consider those may have large enough subgraphs
	...
	/*	
		if(	maxStumps.size() < this.numStump){
			...
			DecisionStump minStump = maxStumps.iterator().next();
			if(stump.getGain() - minStump.getGain() > 1E-6){
				maxStumps.add(stump);
				...
				//System.out.println("max:"+maxGainGraph.getGain()+"\tupperbound:"+boundScore);
				if(boundScore +1E-6< minStump.getGain()){
					//System.out.println("Pruning!");
					for (i = n = 0; i < cnt; i++) { 
						xfs[n++] = null;
					}
		...
		if(this.saveFrequent)
			this.output(frag);/*modify by shirui. output the current fragment */
...
/*------------------------------------------------------------------*/
	...
	protected void mine() throws IOException { /* --- run substructure search */
		...
		t = System.currentTimeMillis() - t;
		this.log.println("[" + k + " visited nodes(s), obtained "+this.maxStumps.size()+" optimal subgraph(s)] done [" + (t / 1000.0)
				+ "s].");
	} /* mine() */
	...
	public  ArrayList<Graph> readGraphs(String file, int mod) throws IOException{
		...
		//System.out.println("Class 1:"+c1+"\tClass 2:"+c2);
		return graphs;
	}
	
	
	public double[] getIndicateVector(Fragment frag){
		int numG = cnts[0]+cnts[1];
		double vector[] = new double[numG];
		
		
		NamedGraph graph = graphs;
		
		Graph sub = (Graph) frag.getGraph().clone();
	    sub.maskTypes(GraphUtils.masks); 
	    sub.prepareEmbed();
		...
		while(graph!=null){
			...
			Graph ng = (Graph) graph.clone();
			ng.maskTypes(GraphUtils.masks);
			ng.prepare();
			if(ng.contains(sub)){
				vector[i] = 1; 
				sup ++;
		...
		//check support
		if(sup!=frag.getSupport()){
			System.err.println("Error: "+this.getClass()+"->Cal support:"+sup+"\tReal Support:"+frag.getSupport());
		}
		
		return vector;
	}
	
	public DecisionStump getMaxGainDecisionStump(ArrayList<Graph> graphs, int support){
/*		String opt = "-t1.0 -s"+support+" -K";
		try {
			
			for(int i = 0; i < graphs.size(); i++){
				NamedGraph ng = new NamedGraph((NamedGraph)graphs.get(i));
				this.addGraph(ng);
			}
			
			
			//System.out.println("Number of graphs in focus:"+graphs.size());
			initMiner(opt.split(" "));
			run();  find substructures, and 
			//stats();
			
		} catch (IOException e) {
			e.printStackTrace();
		}*/
		TreeSet<DecisionStump>  stump = getMaxGainDecisionStumps(graphs,null,support,0.01,1,1);
		return stump.iterator().next();
	}
	
	@Override
	public TreeSet<DecisionStump> getMaxGainDecisionStumps(
			ArrayList<Graph> graphs,ArrayList<Graph> excludedSubs, int support, double wSupp, int numStumps,
			int maxSize, boolean remine) {
		// TODO Auto-generated method stub
		return getMaxGainDecisionStumps(graphs,excludedSubs,support,wSupp,numStumps,maxSize);
	}
	
	public TreeSet<DecisionStump> getMaxGainDecisionStumps(
			ArrayList<Graph> graphs,ArrayList<Graph> excludedSubs, int support, double wSupp, int numStumps,
			int maxSize, boolean remine, int taskSize[]) {
		
		this.taskSize = taskSize;
		System.out.println("Total NUmber of tasks:"+this.taskSize.length);
		// TODO Auto-generated method stub
		return getMaxGainDecisionStumps(graphs,excludedSubs,support,wSupp,numStumps,maxSize);
	}
	 ...
	 * @param support percentage (1-100) of support for subgraphs, if set to -1, no support is setting, algorithm will enumerate all subgraph with pruning rule
	 * @param wSupp minimal weighed score for subgraphs \sum_i^n y_i*lamda_i*h(x_i)
	 * @param numStumps maximal number of subgraphs to return
	 * @param maxSize maximal size of subgraphs (number of nodes)
	 * @return
	 */
	public TreeSet<DecisionStump> getMaxGainDecisionStumps(ArrayList<Graph> graphs, ArrayList<Graph> excludedSubs, int support, double wSupp, int numStumps, int maxSize){
		if(this.taskSize == null){
			taskSize=new int[1];
			taskSize[1] = graphs.size();
		}
		
		this.setAlreadySubGraphs(excludedSubs);
		this.numStump = numStumps;
		String opt = null;
		this.wSupp = wSupp;
		maxStumps = new TreeSet<DecisionStump>();
		if(support == -1){ //omit support
			this.completeSearch = true;
			support = 1;
			System.err.println("Completely Search for Subgraphs!");
		}
		System.out.println("Weighted Support:"+wSupp);
		if(maxSize!=-1)
			opt = "-t1.0 -s"+support+" -g -K -x -n"+maxSize;
		else
			opt = "-t1.0 -s"+support+" -g -K -x";
		try {
			
			for(int i = 0; i < graphs.size(); i++){
				NamedGraph ng =  new NamedGraph((NamedGraph)graphs.get(i));//(NamedGraph)graphs.get(i);//
				this.addGraph(ng);
			}
		...
		//return this.maxGainGraph;
		return this.maxStumps;
	}
	
	@Override
	public TreeSet<DecisionStump> getMaxGainDecisionStumps_LU(
		...
		// TODO Auto-generated method stub
		return getMaxGainDecisionStumps_LU(trainGraphs,uGraphs,excludedSubs,support,ws,numStump,maxPad);
	}
	
	/**get maximum gain strumps from both labeled and unlabeled graphs*/
	public TreeSet<DecisionStump> getMaxGainDecisionStumps_LU(ArrayList<Graph> lGraphs, ArrayList<Graph> uGraphs,ArrayList<Graph> excludedSubs,int support, double wSupp, int numStumps, int maxSize){
		this.numStump = numStumps;
		String opt = null;
		this.wSupp = wSupp;
		maxStumps = new TreeSet<DecisionStump>();
		this.uiversumGraphs = true;
		...
		try {
			...
			for(int i = 0; i < lGraphs.size(); i++){
				NamedGraph ng = new NamedGraph((NamedGraph)lGraphs.get(i));
				this.addGraph(ng);
			}
			...
			for(int i = 0; i < uGraphs.size(); i++){
				...
				NamedGraph ng = new NamedGraph((NamedGraph)uGraphs.get(i));
				
				
				ng.setValue(this.valueForUniversum);
				double value = ng.getValue();
				int grp = (value > 1) ? 1 - this.group : this.group;
				ng.setGroup(grp);
				
				//be careful here, we modify the class value, so that the frequent subgraph mining
				// only perform on labeled graphs, and evaluated on all graphs.
			
				this.addGraph(ng);
			}
			
			System.out.println("Group 1:"+this.cnts[0]+"\tComplement:"+this.cnts[1]);
	...
	public ArrayList<Fragment> getGenerateFrequentGraphs(){
		return this.frequent;
	}
	
	/**
	 * obtain the maximum stumps from frequent subgraphs
	 * @param frequent
	 * @return
	 */
	public TreeSet<DecisionStump> getMaxGainDecisionStumps(ArrayList<Graph> graphs, ArrayList<Fragment> frequent, double wSupp, int num){
	...
	//	TreeSet<DecisionStump> stumps = new TreeSet<DecisionStump>();
		DecisionStump minStump = null;
		for(Fragment frag : frequent){
			DecisionStump stump = this.gainStump(frag);
			
			if(	maxStumps.size() < num){
				maxStumps.add(stump);
			}
			else{
				minStump = maxStumps.iterator().next();
		...
		Iterator<DecisionStump> it = maxStumps.iterator();
		while(it.hasNext()){
			DecisionStump de = it.next();
			if(de.getGain() > wSupp+double_precision){
				break;
			}
			it.remove();
		}
	...
	public void initMiner(String args[]) throws IOException { /*
														 * --- init. a
														 * substructure miner
														 */
		int i, k = 0; /* indices, loop variables */
...
/*		rdr = GraphReader.createReader(new FileReader(datfn),
	...
	public double upperBoundGain(Fragment frag){
		double upV1=0, upV2 = 0;
		double sumOfWeight = 0;
		
		Graph subgraph = frag.getGraph();
		
		NamedGraph graph = graphs;
		int support = 0;
		while(graph!=null){
			
			//mapping calss value [0,1] to [-1,1];
			double classValue = ((NamedGraph) graph).getValue();

			
			sumOfWeight += graph.weight * classValue;
			
			boolean contain = GraphUtils.subgraphIsomorphism(
		...
		if(support!=frag.getSupport()){
			System.err.println("Upper->Cal support:"+support+"\tReal Support:"+frag.getSupport());
		}
		
		//System.out.println("Upper bound:"+(value1 - sumOfWeight)+"\t"+(value1 - sumOfWeight));
		return Math.max(2*upV1 - sumOfWeight, 2*upV2 + sumOfWeight);
	}
	
	public DecisionStump gainStumpMTask(Fragment frag){
		
		
		Graph sub = (Graph) frag.getGraph();
		NamedGraph graph = graphs;
		int support = 0;
		
		int graphIndex = 0;
		int numTask=this.taskSize.length;
		
		double upScores[] = new double[numTask];
		double gainValues[] = new double[numTask];
		
/*	    sub.maskTypes(GraphUtils.masks); 
	    sub.prepareEmbed();*/
		
		//to calculate the discriminative bounds for all task
		double upV1All=0,upV2All=0;
		double sumofWeightAll = 0;
		for(int i = 0; i < numTask; i++){
			double value1=0;
			double upV1=0, upV2 = 0;
			double sumOfWeight = 0;
			
			int numGraphsonTask = getTaskEndId(i);
			//System.out.println("NUMBER OF GRAPHS ON TASK:"+numGraphsonTask);
			int currentGraphs = 0;
			while(graph!=null && graphIndex < numGraphsonTask){
				currentGraphs ++;
				Graph ng = graph;
				//Graph ng = (Graph) graph.clone();
	/*			ng.maskTypes(GraphUtils.masks);
				...
				//mapping calss value [0,1] to [-1,1];
				double classValue = graph.getValue();
				
				boolean contain = ng.contains(sub);
				
				//h(x)=\pi * (2x-1);
				double inva = contain? 1 : 0;
				
				value1 += 1 * graph.weight*classValue* predictValue(inva);
			...
			sumofWeightAll +=sumOfWeight;
			upV1All += upV1;
			upV2All += upV2;
			
			//System.out.println("Current Number of Graphs."+currentGraphs);
			//graphIndex --;
			
			//System.out.println(upV1+","+upV2);
		//	double a = Math.abs(2 * upV1 - sumOfWeight);
		//	double b = Math.abs(2 * upV2 + sumOfWeight);
			
			double upScore = 0;
			if(sumOfWeight >0){
				double a = Math.abs(2 * upV1 - sumOfWeight);
				double b = Math.abs(2 * upV2);
				upScore = Math.max(a, b);
			}else{
				double a = Math.abs(2 * upV2 + sumOfWeight);
				double b = Math.abs(2 * upV1);
				upScore = Math.max(a, b);
			}
			//double upScore = Math.max(a, b);
			upScores[i] = upScore;
		...
		//calculate the bound for discriminative score
		double upScoreAll = 0;
		if(sumofWeightAll >0){
			double a = Math.abs(2 * upV1All- sumofWeightAll);
			double b = Math.abs(2 * upV2All);
			upScoreAll = Math.max(a, b);
		}else{
			double a = Math.abs(2 * upV2All + sumofWeightAll);
			double b = Math.abs(2 * upV1All);
			upScoreAll = Math.max(a, b);
		}
		
		
		//System.out.println(Utils.arrayToString(gainValues));
		if(graph!=null){
			System.err.println("graph is null?:"+(graph == null));
			System.exit(0);
		}
		
		double finalGain =0, finalUpScore = 0;
		finalGain = MathUtils.sum(gainValues);
		finalGain = Math.abs(finalGain); //negative gradient value
		//finalUpScore  = MathUtils.sum(upScores); //sum of seperate upper bound
		finalUpScore = upScoreAll;
		
		
		boolean isViolatedStump = true;
		double qualifyvalue;
		double secondUpScore;
		if("l1".equals(this.regularizationStr)){
			//System.out.println("l1 regularizaiton!");
			qualifyvalue = lmax_norm(gainValues);
			secondUpScore = lmax_norm(upScores);
			//finalGain = MathUtils.sum(gainValues);;
		}else{// l2 
			//System.out.println("l2 regularzion");
			// norm of sub
			qualifyvalue = l2_norm(gainValues);
			secondUpScore=l2_norm(upScores);
		}
		
		if(qualifyvalue < this.mtaskVarepsilon+this.mtaskGamma){
			isViolatedStump = false;
		}
				
		
		//FOR UPPER SCORE
	//	Graph subgraph = (Graph) frag.getGraph();
		
		
		//check support
		if(support!=frag.getSupport()){
			 try {
				 System.err.println(this.getClass()+":gainStumpMtask,Cal support:"+support+"\tReal Support:"+frag.getSupport());
				throw new Exception(this.getClass()+":gainStumpMtask,Cal support:"+support+"\tReal Support:"+frag.getSupport());
		...
		DecisionStump resultStm = new DecisionStump(sub, 1, finalGain,finalUpScore,frag.getSupport(),zeroOneRepresentation);
		resultStm.setValidStump(isViolatedStump);
		resultStm.setSecondUpscores(secondUpScore);
		resultStm.setmTaskNormScore(qualifyvalue);
		
		return resultStm;
	
	}
	
	public DecisionStump gainStump(Fragment frag){
...
/*			ng.maskTypes(GraphUtils.masks);
			...
			//h(x)=\pi * (2x-1);
			double inva = contain? 1 : 0;
/*			value1 += 1 * graph.weight*classValue* 2*(inva-1);
			
			value2 +=-1 *graph.weight*classValue*2*(inva-1);*/
			
			value1 += 1 * graph.weight*classValue* predictValue(inva);
			
			value2 +=-1 *graph.weight*classValue*predictValue(inva);
		...
		if(support!=frag.getSupport()){
			 try {
				 System.err.println(this.getClass()+":Cal support:"+support+"\tReal Support:"+frag.getSupport());
				throw new Exception(this.getClass()+":Cal support:"+support+"\tReal Support:"+frag.getSupport());
		...
		//For general(Any) Boost
		//double a = Math.abs(2 * upV1) +Math.abs(sumOfWeight) ;
		//double b = Math.abs(2 * upV2) +Math.abs( sumOfWeight);
		double upScore = Math.max(a, b);
		
		//double upScore = Math.max(2*upV1 - sumOfWeight, 2*upV2 + sumOfWeight);
		
		//System.out.println("V1:"+value1+",\tV2:"+value2+"\tc1:"+c1+",\tc2:"+c2);
		if(value1 > value2){
			return new DecisionStump(sub, 1, value1,upScore,frag.getSupport(),zeroOneRepresentation);
		}
		else return new DecisionStump(sub, -1, value2,upScore, frag.getSupport(),zeroOneRepresentation);
	}
	
	/*public DecisionStump gainStumpAll(Fragment frag){
		
		int numGraphs = this.cnts[0]+this.cnts[1];
		int containVector[] = new int[numGraphs];
		double classValueVector[] = new double[numGraphs];
		double weightVector[] = new double[numGraphs];
	...
	//	Graph subgraph = (Graph) frag.getGraph();
		while(graph!=null){
			...
			//mapping calss value [0,1] to [-1,1];
			double classValue = graph.getValue();
			classValueVector[graphIndex] = classValue;
			
			boolean contain = ng.contains(sub);
			
			//h(x)=\pi * (2x-1);
			double inva = contain? 1 : 0;
			containVector[graphIndex] = contain? 1 : -1;
			weightVector[graphIndex] = ng.weight;
		...
		double gainValues[] = new double[numTask];
		graphIndex = 0;
		for(int i = 0; i < numTask; i++,graphIndex++){
			int numGraphsonTask = getTaskEndId(i);
			double scoreValue = 0, upVp=0, upVn=0;
			if(i == numGraphsonTask){
				gainValues[i] = Math.abs(scoreValue);
				
				double a = Math.abs(2 * upVp) +Math.abs(sumOfWeight) ;
				double b = Math.abs(2 * upVn) +Math.abs( sumOfWeight);
				upScores[i] = Math.max(a, b);
				
				scoreValue = 0; upVp = 0; upVn = 0;
				
			}
			scoreValue += pi * weightVector[graphIndex]*classValueVector[graphIndex];
			
			if(classValueVector[graphIndex]==1){
				upVp += weightVector[graphIndex];
			}else{
				upVn += weightVector[graphIndex];
			}
			
		}
		double upScore =0;
		if("l1".equals(this.regularization)){
			upScore = MathUtils.sum(upScores);
		}else{ //l2 norm
			upScore = this.l2_norm(upScores);
		}
		
		double gainScore = MathUtils.sum(gainValues);
		...
		return new DecisionStump(sub, pi, gainScore,upScore, frag.getSupport(),zeroOneRepresentation);
	...
	private int getTaskEndId(int taskId){
		int x=0;
	...
	private double l2_norm(double v[]){
		double values = 0;
	...
	private double lmax_norm(double v[]){
		double values = Double.MIN_VALUE;
		for(int i = 0; i <v.length; i++){
			double vx = Math.abs(v[i]);
			if(vx > values)
				values = vx;
		}
	...
	private double predictValue(double inva){
		//return (2 * inva - 1);
	...
	public DecisionStump gainStump_LU(Fragment frag){
		...
		//FOR UPPER SCORE
		double upV1=0, upV2 = 0, upV1_u=0, upV2_u=0;
		double sumOfWeight = 0, sumOfWeight_u=0;
...
/*			Graph ng = (Graph) graph.clone();
			...
			if(classValue!=this.valueForUniversum){ //labeled
			
				value1 += 1 * graph.weight*classValue*predictValue(inva);
				
				value2 +=-1 *graph.weight*classValue*predictValue(inva);
				sumOfWeight += graph.weight * classValue;
			}else{ //for universum graphs, its weight = p_j, weight2=b_j
				double v = graph.weight - graph.weight2; 
				value1 += 1 * v* (2*inva-1);
			...
			if(contain){
				support++;
				if(classValue !=0 ){ //labeled graphs
					if(classValue == 1){
						upV1 += graph.weight;
					}
					else{
				...
				}else{//universum graphs
					upV1_u += graph.weight;
					upV2_u += graph.weight2;
				}
			}
		...
		//check support
		if(support!=frag.getSupport()){
			System.err.println("Cal support:"+support+"\tReal Support:"+frag.getSupport());
		}
		...
		if(!uiversumGraphs){
			...
			double theta1 = 2*(upV1+upV1_u)+sumOfWeight_u-sumOfWeight;
			double theta2 = 2*(upV2+upV2_u)-sumOfWeight_u+sumOfWeight;
			upScore = Math.max(theta1, theta2);
		}
	...
	public void evaluateStumpList(ArrayList<Graph> graphs, ArrayList<Graph> ugraphs, DecisionStump stumpt){
	...
	private boolean isUnique(DecisionStump stmp){
		
		if(this.existedSubgraphs.size() == 0)
			return true;
		
		Graph g = (Graph) stmp.getGraphFeature().clone();
		g.decode();
		g.prepare();
		
		for(Graph exg : existedSubgraphs){
			//exg.decode();
			//exg.prepareEmbed();
			if(g.equals(exg)){
			//	System.out.println("Graph already existed!!!");
				return false;
			}
	...
	public void setAlreadySubGraphs(ArrayList<Graph> graphs){
		if(graphs == null)
			return;
		this.existedSubgraphs = graphs;
		for(Graph g : existedSubgraphs){
			g.decode();
			g.prepare();
		}
	}
	...
	public static void main(String args[]) { /* --- main function */
		...
		GraphBoostMinerGraphsMtask miner = new GraphBoostMinerGraphsMtask(); /* substructure miner */
		//miner.saveFrequent = true;
		//miner.completeSearch = true;
...
/*			miner.init(args);  initialize the miner, 
			...
			//ArrayList<Graph>  graphs = miner.readGraphs("data/175-balance800.sdf", GraphReader.GRAPHS);
			//ArrayList<Graph>  graphs = miner.readGraphs("ptc_data/PTC_pn_MM.smi", GraphReader.GRAPHS);
			ArrayList<Graph>  graphs = miner.readGraphs("ptc_data/OKD_all.sdf", GraphReader.GRAPHS);
			//set weight
	/*		for(int i = 0; i<graphs.size(); i++){
				graphs.get(i).setWeight(1.0/graphs.size());
			}*/
			
			
			//split data into training and test
			ArrayList<Graph>  train = new ArrayList<Graph>();
			ArrayList<Graph> uGraphs = new ArrayList<Graph>();
			...
			for(int i = 0; i < graphs.size(); i++){
				...
				if(ng.getValue() == 0){ //Universum Graphs
					uGraphs.add(ng);
				}
				else{ //Labeled Graphs cut into training and test
					train.add(ng);
				}
			}
			
			System.out.println("size of label: "+train.size());
			System.out.println("size of universum: "+uGraphs.size());
			//weight
			for(int i = 0; i < train.size(); i++){
				train.get(i).setWeight(1.0/train.size());
			}
			for(int i = 0; i < uGraphs.size(); i++){
				Graph g = uGraphs.get(i);
				g.setWeight(1.0/uGraphs.size());
				g.setWeight2(1.0/uGraphs.size());
			}
			//System.out.println(uGraphs.get(0).weight);
			
			System.out.println("Number of labeled graphs:"+train.size());
			TreeSet<DecisionStump> stump = miner.getMaxGainDecisionStumps_LU(train, uGraphs, null,-1,0, 20,10);
			
			System.out.println("Number of stumps:"+stump.size());
			Iterator<DecisionStump> it = stump.descendingIterator();
			
			ArrayList<Graph> select = new ArrayList<Graph>();
			//evaluate
			for(int i = 0; i < stump.size(); i++){
				DecisionStump st = it.next();
				select.add(st.getGraphFeature());
				//System.out.println(st.getGain());
				miner.evaluateStumpList(train,uGraphs,st);
			}
			System.out.println("---------------------------------------");
			//System.out.println(miner.supp);
			
			ArrayList<Fragment> frequent = miner.getGenerateFrequentGraphs();
			System.out.println("Number of frequent:"+frequent.size());
			
			

			miner = new GraphBoostMinerGraphsMtask();
			miner.setAlreadySubGraphs(select);
			TreeSet<DecisionStump> stump2 = miner.getMaxGainDecisionStumps_LU(train, uGraphs,null, -1,0, 20,10);
			it = stump2.descendingIterator();
			
			ArrayList<Graph> select2 = new ArrayList<Graph>();
			//evaluate
			for(int i = 0; i < stump2.size(); i++){
				DecisionStump st = it.next();
				select2.add(st.getGraphFeature());
				//System.out.println(st.getGain());
				miner.evaluateStumpList(train,uGraphs,st);
				//miner.evaluateStumpList(graphs,st);
			}
			
			System.out.println(select.get(1).equals(select2.get(1)));
